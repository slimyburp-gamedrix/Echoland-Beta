...
const CACHE_FILE = path.join(CACHE_DIR, "area-index.json");
...
if (await cacheFile.exists()) {
    try {
        ...
        areaIndex.push(...cached.areaIndex);
        for (const [key, value] of cached.areaByUrlName) {
            areaByUrlName.set(key, value);
        }
        console.log(`✓ Loaded ${areaIndex.length} areas from cache`);
        cacheLoaded = true;
    } catch (error) {
        console.log("Cache invalid, rebuilding index...");
    }
}
...
if (!cacheLoaded) {
    ...
    console.log(`Building area index from ${totalFiles} files...`);
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (i % 1000 === 0) {
            const percentage = ((i + 1) / totalFiles * 100).toFixed(1);
            console.log(`Indexed ${i} areas (${percentage}%)...`);
        }
    ...
    console.log(`✓ Indexed ${areaIndex.length} areas (100%)`);
    
    // Save to cache
    console.log("Saving index to cache...");
    const cacheData = {
        areaIndex,
        areaByUrlName: Array.from(areaByUrlName.entries()),
